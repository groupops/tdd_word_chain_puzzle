...
public class WordChainPuzzleSolver {
  ...
  private String nextWord(String currentWord, String endWord){
    for (int i = 0; i < currentWord.length(); i++) {
      ...
      if (currentWord.charAt(i) != endWord.charAt(i)) {
        // try to change the letter and check dict
        String nextWord = currentWordMutable.replace(i, i+1, String.valueOf(endWord.charAt(i))).toString();
  ...
  private void validateInputWords(String startWord, String endWord){
    if(startWord.equals(endWord))
      throw new IllegalArgumentException("The start and end words are the same word");
    if (startWord.length() != endWord.length())
      throw new IllegalArgumentException("The start and end word do not have the same length");

    if (!isWordInDictionary(startWord))
      throw new WordIsNotInDictionaryException("The start word is not in the dictionary");

    if (!isWordInDictionary(endWord))
      throw new WordIsNotInDictionaryException("The end word is not in the dictionary");
